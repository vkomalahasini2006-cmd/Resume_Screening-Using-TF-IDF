...
def load_dataset_and_screen(csv_path: str, limit: int = 100) -> None:
    """Load the Kaggle resume dataset CSV and run the screening pipeline."""
    resumes, df = load_kaggle_resume_dataset(csv_path, max_samples=limit)
    print(f"Loaded {len(resumes)} resumes from {csv_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the resume screening pipeline on sample resumes or a Kaggle resume dataset."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        help="Path to the Kaggle Resume.csv file to screen.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=100,
        help="Maximum number of resumes to load from the dataset.",
    )
    args = parser.parse_args()

    if args.dataset:
        load_dataset_and_screen(args.dataset, limit=args.limit)
    else:
        ranked_df = screen_resumes(JOB_DESCRIPTION, RESUMES)
        print_report(ranked_df)
        ranked_df.to_csv("ranked_candidates.csv")
